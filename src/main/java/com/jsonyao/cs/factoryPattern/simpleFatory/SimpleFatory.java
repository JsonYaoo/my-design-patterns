 ...
 *      => 具体的实现类相当于抽象业务的落地
 * C. 优点:
 *      a. 简单工厂模式通过这种做法实现了对责任的分割, 专门的工厂类用于创建产品, 客户端免除了除直接创建产品对象的责任, 而仅仅是消费产品
 *      b. 客户端无须知道所创建的具体产品类的类名, 只需要知道具体产品类的对应参数即可
 *      c. 通过引入配置文件, 可以在不修改任何客户端代码的情况下更换和增加新的具体产品类, 在一定程度上提高了系统的灵活性
 * D. 缺点:
